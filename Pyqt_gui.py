# -*- coding: utf-8 -*-

# Form implementation generated from reading ui file 'main_window_black.ui'
#
# Created by: PyQt5 UI code generator 5.15.4
#
# WARNING: Any manual changes made to this file will be lost when pyuic5 is
# run again.  Do not edit this file unless you know what you are doing.


from PyQt5 import QtCore, QtGui, QtWidgets

def getScaleRate(w, h):
    desktop = QtWidgets.QApplication.desktop().screenGeometry()
    screenWidth = desktop.width()
    screenHeight = desktop.height()
    if screenWidth > screenHeight:
        return (screenHeight * 0.87) / h
    else:
        return (screenWidth * 0.87) / w

class Ui_SIC(object):
    def getScaleValue(self, v):
        result = v * self.scaleRate
        if result > 16777215:
            return 16777215
        else:
            return result
    def getScaleSize(self, w, h):
        return QtCore.QSize(self.getScaleValue(w), self.getScaleValue(h))
    def getScaleRect(self, x, y, w, h):
        return QtCore.QRect(self.getScaleValue(x), self.getScaleValue(y), self.getScaleValue(w), self.getScaleValue(h))
    def setupUi(self, SIC):
        SIC.setObjectName("SIC")
        defaultWidth = 1000
        defaultHeight = 900
        self.scaleRate = getScaleRate(defaultWidth, defaultHeight)
        # print(self.scaleRate)
        SIC.resize(self.getScaleValue(defaultWidth), self.getScaleValue(defaultHeight))
        SIC.setMinimumSize(50, 50)
        icon = QtGui.QIcon()
        icon.addPixmap(QtGui.QPixmap("res/icon.ico"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        SIC.setWindowIcon(icon)
        SIC.setStyleSheet("QMenuBar {\n"
"    background-color: rgb(35, 37, 41);\n"
"    color: white;\n"
"    border-color: rgb(60, 85, 109);\n"
"    gridline-color:  rgb(60, 85, 109);\n"
"}\n"
"\n"
"QMenuBar::item {\n"
"    spacing: 3px;           \n"
"    padding: 2px 10px;\n"
"    background-color: rgb(35, 37, 41);\n"
"    color: white;\n"
"    border-radius: 5px;\n"
"}\n"
"QMenuBar::item:selected {    \n"
"    background-color: rgb(244,164,96);\n"
"    color: white;\n"
"}\n"
"QMenuBar::item:pressed {\n"
"    background: rgb(128,0,0);\n"
"}\n"
"\n"
"/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */  \n"
"\n"
"QMenu {\n"
"    background-color: rgb(35, 37, 41);\n"
"    border: 1px solid black;\n"
"    margin: 2px;\n"
"    color: white;\n"
"}\n"
"QMenu::item {\n"
"    background-color: rgb(35, 37, 41);\n"
"}\n"
"QMenu::item:selected { \n"
"    background-color: rgb(149, 165, 166);\n"
"    color: rgb(220,220,220);\n"
"}")
        self.centralwidget = QtWidgets.QWidget(SIC)
        self.centralwidget.setObjectName("centralwidget")
        self.frame = QtWidgets.QFrame(self.centralwidget)
        self.frame.setGeometry(self.getScaleRect(0, -10, 971, 891))
        sizePolicy = QtWidgets.QSizePolicy(QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Expanding)
        sizePolicy.setHorizontalStretch(99)
        sizePolicy.setVerticalStretch(99)
        sizePolicy.setHeightForWidth(self.frame.sizePolicy().hasHeightForWidth())
        self.frame.setSizePolicy(sizePolicy)
        self.frame.setAutoFillBackground(False)
        self.frame.setStyleSheet("QProgressBar {\n"
"    \n"
"    background-color: rgb(96, 116, 165);\n"
"    /*color: white;*/\n"
"    color: black;\n"
"    border-style: none;\n"
"    text-align: center;\n"
"    border-radius: 10px;\n"
"}\n"
"\n"
"QAbstractScrollArea{ \n"
"    background-color: rgb(149, 165, 166);\n"
"    border: none;\n"
"\n"
"}\n"
"\n"
"QProgressBar::chunk {\n"
"    border-radius: 10px;\n"
"    \n"
"    color: rgb(255, 255, 255);\n"
"    background-color: qlineargradient(spread:pad, x1:0, y1:0.534, x2:1, y2:0.522, stop:0 rgba(46, 68, 91, 255), stop:1 rgba(101, 151, 200, 255));\n"
"}\n"
"\n"
"QTabWidget {\n"
"    background-color: rgb(91, 129, 165);\n"
"    color: black;\n"
"    border-color: rgb(60, 85, 109);\n"
"    gridline-color:  rgb(60, 85, 109);\n"
"    font: 8pt \"Segoe MDL2 Assets\";\n"
"}\n"
"\n"
"QListWidget {\n"
"    background-color: rgb(79, 104, 191);\n"
"    color: white;\n"
"    border-color: rgb(60, 85, 109);\n"
"    gridline-color:  rgb(60, 85, 109);\n"
"font: 10pt \"Segoe MDL2 Assets\";\n"
"}\n"
"QListView::item {\n"
"    height: 30px;\n"
"    width: 30px;\n"
"}\n"
"QLineEdit {\n"
"    background-color: rgb(149, 165, 166);\n"
"    color: white;\n"
"    border: none;\n"
"font: 10pt \"Segoe MDL2 Assets\";\n"
"}\n"
"\n"
"QTabBar::tab {\n"
"    background: rgb(81, 85, 93);\n"
"    color: white;\n"
"}\n"
" QTabBar::tab:!selected {\n"
"    background-color: rgb(138, 138, 138);\n"
"    color: white;\n"
"    }\n"
"\n"
"QFrame{\n"
"    background-color: rgb(47, 49, 54);\n"
"    color: white;\n"
"    \n"
"    font: 10pt \"Segoe MDL2 Assets\";\n"
"}\n"
"\n"
"QTextEdit {\n"
"    background-color: rgb(149, 165, 166);\n"
"font: 10pt \"Segoe MDL2 Assets\";\n"
"}\n"
"\n"
"QLabel {\n"
"font: 10pt \"Segoe MDL2 Assets\";\n"
"}\n"
"\n"
"QComboBox {\n"
"    background-color: rgb(149, 165, 166);\n"
"    border: none;\n"
"    font: 10pt \"Segoe MDL2 Assets\";\n"
"    color: white;\n"
"}\n"
"\n"
"    QComboBox QAbstractItemView::item\n"
"    {\n"
"      padding-top: 10px;\n"
"      padding-bottom: 10px;\n"
"    }\n"
"\n"
"QRadioButton {\n"
"    color: white;\n"
"    border-color: rgb(60, 85, 109);\n"
"    gridline-color:  rgb(60, 85, 109);\n"
"    font: 10pt \"Segoe MDL2 Assets\";\n"
"}\n"
"QCheckBox {\n"
"    color: white;\n"
"    border-color: rgb(60, 85, 109);\n"
"    gridline-color:  rgb(60, 85, 109);\n"
"    font: 10pt \"Segoe MDL2 Assets\";\n"
"}\n"
"QTabWidget::pane { \n"
"border: 1; \n"
"border-color: rgb(255, 0, 0);\n"
"}\n"
"\n"
"\n"
"QPushButton {\n"
"    background-color: rgb(255, 173, 0);\n"
"border-radius: 10px;\n"
"font: 11pt \"Segoe MDL2 Assets\";\n"
"    border: none;\n"
"font-weight: bold\n"
"}\n"
"\n"
"QPushButton::pressed\n"
"                             {\n"
"                             background-color : rgb(244, 207, 0);\n"
"                             }\n"
"QMenuBar::item:pressed\n"
"                             {\n"
"                             background-color : rgb(149, 165, 166);\n"
"                             }\n"
"")
        self.frame.setFrameShape(QtWidgets.QFrame.StyledPanel)
        self.frame.setFrameShadow(QtWidgets.QFrame.Raised)
        self.frame.setObjectName("frame")
        self.gridLayout_3 = QtWidgets.QGridLayout(self.frame)
        self.gridLayout_3.setObjectName("gridLayout_3")
        self.gridLayout_2 = QtWidgets.QGridLayout()
        self.gridLayout_2.setObjectName("gridLayout_2")
        self.Right_column = QtWidgets.QFrame(self.frame)
        self.Right_column.setObjectName("Right_column")
        self.gridLayout = QtWidgets.QGridLayout(self.Right_column)
        self.gridLayout.setObjectName("gridLayout")
        self.edit = QtWidgets.QPushButton(self.Right_column)
        self.edit.setMinimumSize(self.getScaleSize(200, 25))
        self.edit.setMaximumSize(self.getScaleSize(16777215, 16777215))
        self.edit.setObjectName("edit")
        self.gridLayout.addWidget(self.edit, 3, 0, 1, 1, QtCore.Qt.AlignHCenter)
        self.Mod_content = QtWidgets.QListWidget(self.Right_column)
        self.Mod_content.setMaximumSize(self.getScaleSize(250, 16777215))
        self.Mod_content.setStyleSheet("background-color: rgb(46, 68, 91);\n"
"    color: rgb(220, 220, 220);")
        self.Mod_content.setObjectName("Mod_content")
        self.gridLayout.addWidget(self.Mod_content, 2, 0, 1, 1)
        self.label_24 = QtWidgets.QLabel(self.Right_column)
        self.label_24.setObjectName("label_24")
        self.gridLayout.addWidget(self.label_24, 1, 0, 1, 1)
        self.Clear_list = QtWidgets.QPushButton(self.Right_column)
        self.Clear_list.setMinimumSize(self.getScaleSize(200, 25))
        self.Clear_list.setMaximumSize(self.getScaleSize(16777215, 16777215))
        self.Clear_list.setObjectName("Clear_list")
        self.gridLayout.addWidget(self.Clear_list, 4, 0, 1, 1, QtCore.Qt.AlignHCenter)
        self.Options = QtWidgets.QPushButton(self.Right_column)
        self.Options.setMinimumSize(self.getScaleSize(200, 25))
        self.Options.setMaximumSize(self.getScaleSize(16777215, 16777215))
        self.Options.setObjectName("Options")
        self.gridLayout.addWidget(self.Options, 6, 0, 1, 1, QtCore.Qt.AlignHCenter)
        self.Remove_from_mod = QtWidgets.QPushButton(self.Right_column)
        self.Remove_from_mod.setMinimumSize(self.getScaleSize(200, 25))
        self.Remove_from_mod.setMaximumSize(self.getScaleSize(16777215, 16777215))
        self.Remove_from_mod.setObjectName("Remove_from_mod")
        self.gridLayout.addWidget(self.Remove_from_mod, 5, 0, 1, 1, QtCore.Qt.AlignHCenter)
        self.patreon = QtWidgets.QToolButton(self.Right_column)
        self.patreon.setMinimumSize(self.getScaleSize(80, 80))
        self.patreon.setMaximumSize(self.getScaleSize(16777215, 80))
        self.patreon.setStyleSheet("background-image : url(res/patreon.png);\n"
"background-color: rgb(47, 49, 54);\n"
"border: none;")
        self.patreon.setText("")
        icon1 = QtGui.QIcon()
        icon1.addPixmap(QtGui.QPixmap("res/ppatreon.png"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        self.patreon.setIcon(icon1)
        self.patreon.setIconSize(self.getScaleSize(180, 100))
        self.patreon.setObjectName("patreon")
        self.gridLayout.addWidget(self.patreon, 7, 0, 1, 1)
        self.gridLayout_2.addWidget(self.Right_column, 1, 1, 1, 1)
        self.progressBar = QtWidgets.QProgressBar(self.frame)
        self.progressBar.setMinimumSize(self.getScaleSize(0, 35))
        self.progressBar.setProperty("value", 0)
        self.progressBar.setTextVisible(True)
        self.progressBar.setOrientation(QtCore.Qt.Horizontal)
        self.progressBar.setFormat("")
        self.progressBar.setObjectName("progressBar")
        self.gridLayout_2.addWidget(self.progressBar, 4, 0, 1, 2)
        self.Create_mod = QtWidgets.QPushButton(self.frame)
        self.Create_mod.setMinimumSize(self.getScaleSize(0, 30))
        self.Create_mod.setObjectName("Create_mod")
        self.gridLayout_2.addWidget(self.Create_mod, 3, 0, 1, 2)
        self.horizontalLayout_4 = QtWidgets.QHBoxLayout()
        self.horizontalLayout_4.setObjectName("horizontalLayout_4")
        self.formLayout_9 = QtWidgets.QFormLayout()
        self.formLayout_9.setObjectName("formLayout_9")
        self.label_83 = QtWidgets.QLabel(self.frame)
        self.label_83.setMinimumSize(self.getScaleSize(100, 0))
        self.label_83.setObjectName("label_83")
        self.formLayout_9.setWidget(0, QtWidgets.QFormLayout.LabelRole, self.label_83)
        self.Lang = QtWidgets.QComboBox(self.frame)
        self.Lang.setMinimumSize(self.getScaleSize(120, 25))
        self.Lang.setMaximumSize(self.getScaleSize(200, 16777215))
        self.Lang.setObjectName("Lang")
        self.formLayout_9.setWidget(0, QtWidgets.QFormLayout.FieldRole, self.Lang)
        self.label_84 = QtWidgets.QLabel(self.frame)
        self.label_84.setMinimumSize(self.getScaleSize(100, 0))
        self.label_84.setObjectName("label_84")
        self.formLayout_9.setWidget(1, QtWidgets.QFormLayout.LabelRole, self.label_84)
        self.pack_name = QtWidgets.QLineEdit(self.frame)
        self.pack_name.setMinimumSize(self.getScaleSize(120, 25))
        self.pack_name.setMaximumSize(self.getScaleSize(200, 25))
        self.pack_name.setObjectName("pack_name")
        self.formLayout_9.setWidget(1, QtWidgets.QFormLayout.FieldRole, self.pack_name)
        self.horizontalLayout_4.addLayout(self.formLayout_9)
        self.formLayout_4 = QtWidgets.QFormLayout()
        self.formLayout_4.setObjectName("formLayout_4")
        self.label_14 = QtWidgets.QLabel(self.frame)
        self.label_14.setMinimumSize(self.getScaleSize(0, 25))
        self.label_14.setObjectName("label_14")
        self.formLayout_4.setWidget(0, QtWidgets.QFormLayout.LabelRole, self.label_14)
        self.horizontalLayout_2 = QtWidgets.QHBoxLayout()
        self.horizontalLayout_2.setObjectName("horizontalLayout_2")
        self.wiiu_radiobutton = QtWidgets.QRadioButton(self.frame)
        self.wiiu_radiobutton.setMinimumSize(self.getScaleSize(0, 20))
        self.wiiu_radiobutton.setChecked(True)
        self.wiiu_radiobutton.setObjectName("wiiu_radiobutton")
        self.horizontalLayout_2.addWidget(self.wiiu_radiobutton)
        self.switch_radiobutton = QtWidgets.QRadioButton(self.frame)
        self.switch_radiobutton.setEnabled(True)
        self.switch_radiobutton.setMinimumSize(self.getScaleSize(0, 20))
        self.switch_radiobutton.setObjectName("switch_radiobutton")
        self.horizontalLayout_2.addWidget(self.switch_radiobutton)
        self.formLayout_4.setLayout(0, QtWidgets.QFormLayout.FieldRole, self.horizontalLayout_2)
        self.label_85 = QtWidgets.QLabel(self.frame)
        self.label_85.setMinimumSize(self.getScaleSize(100, 0))
        self.label_85.setObjectName("label_85")
        self.formLayout_4.setWidget(1, QtWidgets.QFormLayout.LabelRole, self.label_85)

        self.shop = QtWidgets.QComboBox(self.frame)
        self.shop.setMinimumSize(self.getScaleSize(180, 25))
        self.shop.setMaximumSize(self.getScaleSize(185, 0))
        self.shop.setObjectName("shop")
        # self.formLayout_4.setWidget(1, QtWidgets.QFormLayout.FieldRole, self.shop)
        self.shop.hide()

        def shop_name_change(i):
            self.shop.setCurrentIndex(i)

        self.shop_name = QtWidgets.QComboBox(self.frame)
        self.shop_name.setMinimumSize(self.getScaleSize(180, 25))
        self.shop_name.setMaximumSize(self.getScaleSize(185, 0))
        self.shop_name.setObjectName("shop_name")
        self.formLayout_4.setWidget(1, QtWidgets.QFormLayout.FieldRole, self.shop_name)
        self.shop_name.currentIndexChanged.connect(shop_name_change)

        self.horizontalLayout_4.addLayout(self.formLayout_4)
        self.gridLayout_2.addLayout(self.horizontalLayout_4, 0, 0, 1, 1)
        self.TabWidget = QtWidgets.QTabWidget(self.frame)
        sizePolicy = QtWidgets.QSizePolicy(QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Expanding)
        sizePolicy.setHorizontalStretch(0)
        sizePolicy.setVerticalStretch(0)
        sizePolicy.setHeightForWidth(self.TabWidget.sizePolicy().hasHeightForWidth())
        self.TabWidget.setSizePolicy(sizePolicy)
        self.TabWidget.setMinimumSize(self.getScaleSize(720, 580))
        self.TabWidget.setLayoutDirection(QtCore.Qt.LeftToRight)
        self.TabWidget.setStyleSheet("QTabWidget {\n"
"    background-color: rgb(91, 129, 165);\n"
"    color: rgb(60, 85, 109);\n"
"    border-color: rgb(60, 85, 109);\n"
"    gridline-color:  rgb(60, 85, 109);\n"
"    font: 8pt \"Segoe MDL2 Assets\";\n"
"}")
        self.TabWidget.setObjectName("TabWidget")
        self.tab = QtWidgets.QWidget()
        self.tab.setObjectName("tab")
        self.frame_4 = QtWidgets.QFrame(self.tab)
        self.frame_4.setGeometry(self.getScaleRect(0, 0, 721, 701))
        self.frame_4.setStyleSheet("QFrame{\n"
"    background-color: rgb(81, 85, 93);\n"
"    color: white;\n"
"    \n"
"    font: 9pt \"Segoe MDL2 Assets\";\n"
"}")
        self.frame_4.setFrameShape(QtWidgets.QFrame.StyledPanel)
        self.frame_4.setFrameShadow(QtWidgets.QFrame.Raised)
        self.frame_4.setObjectName("frame_4")
        self.horizontalLayout = QtWidgets.QHBoxLayout(self.frame_4)
        self.horizontalLayout.setObjectName("horizontalLayout")
        self.gridLayout_5 = QtWidgets.QGridLayout()
        self.gridLayout_5.setObjectName("gridLayout_5")
        self.formLayout = QtWidgets.QFormLayout()
        self.formLayout.setObjectName("formLayout")
        self.label_28 = QtWidgets.QLabel(self.frame_4)
        self.label_28.setObjectName("label_28")
        self.formLayout.setWidget(0, QtWidgets.QFormLayout.LabelRole, self.label_28)
        self.base = QtWidgets.QComboBox(self.frame_4)
        self.base.setEnabled(True)
        self.base.setMinimumSize(self.getScaleSize(260, 25))
        self.base.setMaximumSize(self.getScaleSize(166666, 16777215))
        self.base.setObjectName("base")
        self.formLayout.setWidget(0, QtWidgets.QFormLayout.FieldRole, self.base)
        self.label_4 = QtWidgets.QLabel(self.frame_4)
        self.label_4.setObjectName("label_4")
        self.formLayout.setWidget(1, QtWidgets.QFormLayout.LabelRole, self.label_4)
        self.name = QtWidgets.QLineEdit(self.frame_4)
        self.name.setMinimumSize(self.getScaleSize(0, 25))
        self.name.setMaximumSize(self.getScaleSize(16777215, 25))
        self.name.setObjectName("name")
        self.formLayout.setWidget(1, QtWidgets.QFormLayout.FieldRole, self.name)
        self.label_5 = QtWidgets.QLabel(self.frame_4)
        self.label_5.setObjectName("label_5")
        self.formLayout.setWidget(3, QtWidgets.QFormLayout.LabelRole, self.label_5)
        self.defence = QtWidgets.QLineEdit(self.frame_4)
        self.defence.setMinimumSize(self.getScaleSize(0, 25))
        self.defence.setMaximumSize(self.getScaleSize(16777215, 25))
        self.defence.setObjectName("defence")
        self.formLayout.setWidget(3, QtWidgets.QFormLayout.FieldRole, self.defence)
        self.label_6 = QtWidgets.QLabel(self.frame_4)
        self.label_6.setObjectName("label_6")
        self.formLayout.setWidget(4, QtWidgets.QFormLayout.LabelRole, self.label_6)
        self.elink = QtWidgets.QLineEdit(self.frame_4)
        self.elink.setMinimumSize(self.getScaleSize(0, 25))
        self.elink.setMaximumSize(self.getScaleSize(16777215, 25))
        self.elink.setObjectName("elink")
        self.formLayout.setWidget(4, QtWidgets.QFormLayout.FieldRole, self.elink)
        self.label_7 = QtWidgets.QLabel(self.frame_4)
        self.label_7.setObjectName("label_7")
        self.formLayout.setWidget(5, QtWidgets.QFormLayout.LabelRole, self.label_7)
        self.slink = QtWidgets.QLineEdit(self.frame_4)
        self.slink.setMinimumSize(self.getScaleSize(0, 25))
        self.slink.setMaximumSize(self.getScaleSize(16777215, 25))
        self.slink.setObjectName("slink")
        self.formLayout.setWidget(5, QtWidgets.QFormLayout.FieldRole, self.slink)
        self.label_8 = QtWidgets.QLabel(self.frame_4)
        self.label_8.setObjectName("label_8")
        self.formLayout.setWidget(6, QtWidgets.QFormLayout.LabelRole, self.label_8)
        self.profile = QtWidgets.QComboBox(self.frame_4)
        self.profile.setMinimumSize(self.getScaleSize(0, 25))
        self.profile.setObjectName("profile")
        self.formLayout.setWidget(6, QtWidgets.QFormLayout.FieldRole, self.profile)
        self.label_9 = QtWidgets.QLabel(self.frame_4)
        self.label_9.setObjectName("label_9")
        self.formLayout.setWidget(7, QtWidgets.QFormLayout.LabelRole, self.label_9)
        self.series = QtWidgets.QComboBox(self.frame_4)
        self.series.setMinimumSize(self.getScaleSize(0, 25))
        self.series.setObjectName("series")
        self.formLayout.setWidget(7, QtWidgets.QFormLayout.FieldRole, self.series)
        self.label_10 = QtWidgets.QLabel(self.frame_4)
        self.label_10.setObjectName("label_10")
        self.formLayout.setWidget(8, QtWidgets.QFormLayout.LabelRole, self.label_10)
        self.label_11 = QtWidgets.QLabel(self.frame_4)
        self.label_11.setObjectName("label_11")
        self.formLayout.setWidget(9, QtWidgets.QFormLayout.LabelRole, self.label_11)
        self.name_desc = QtWidgets.QLineEdit(self.frame_4)
        self.name_desc.setMinimumSize(self.getScaleSize(0, 25))
        self.name_desc.setMaximumSize(self.getScaleSize(16777215, 25))
        self.name_desc.setObjectName("name_desc")
        self.formLayout.setWidget(9, QtWidgets.QFormLayout.FieldRole, self.name_desc)
        self.label_12 = QtWidgets.QLabel(self.frame_4)
        self.label_12.setObjectName("label_12")
        self.formLayout.setWidget(10, QtWidgets.QFormLayout.LabelRole, self.label_12)
        self.physics = QtWidgets.QLineEdit(self.frame_4)
        self.physics.setMinimumSize(self.getScaleSize(0, 25))
        self.physics.setMaximumSize(self.getScaleSize(16777215, 25))
        self.physics.setObjectName("physics")
        self.formLayout.setWidget(10, QtWidgets.QFormLayout.FieldRole, self.physics)
        self.price = QtWidgets.QLineEdit(self.frame_4)
        self.price.setMinimumSize(self.getScaleSize(0, 25))
        self.price.setMaximumSize(self.getScaleSize(16777215, 25))
        self.price.setObjectName("price")
        self.formLayout.setWidget(8, QtWidgets.QFormLayout.FieldRole, self.price)
        self.upgradeable = QtWidgets.QCheckBox(self.frame_4)
        self.upgradeable.setMinimumSize(self.getScaleSize(0, 25))
        self.upgradeable.setObjectName("upgradeable")
        self.formLayout.setWidget(2, QtWidgets.QFormLayout.FieldRole, self.upgradeable)
        self.gridLayout_5.addLayout(self.formLayout, 0, 0, 1, 1)
        self.formLayout_6 = QtWidgets.QFormLayout()
        self.formLayout_6.setObjectName("formLayout_6")
        self.label_13 = QtWidgets.QLabel(self.frame_4)
        self.label_13.setObjectName("label_13")
        self.formLayout_6.setWidget(0, QtWidgets.QFormLayout.LabelRole, self.label_13)
        self.desc = QtWidgets.QTextEdit(self.frame_4)
        self.desc.setMinimumSize(self.getScaleSize(0, 80))
        self.desc.setMaximumSize(self.getScaleSize(16777215, 100))
        self.desc.setStyleSheet("background-color: rgb(149, 165, 166);\n"
"font: 10pt \"Segoe MDL2 Assets\";")
        self.desc.setObjectName("desc")
        self.formLayout_6.setWidget(0, QtWidgets.QFormLayout.FieldRole, self.desc)
        self.gridLayout_5.addLayout(self.formLayout_6, 1, 0, 1, 1)
        self.horizontalLayout.addLayout(self.gridLayout_5)
        self.horizontalLayout_5 = QtWidgets.QHBoxLayout()
        self.horizontalLayout_5.setObjectName("horizontalLayout_5")
        self.formLayout_2 = QtWidgets.QFormLayout()
        self.formLayout_2.setObjectName("formLayout_2")
        self.label_15 = QtWidgets.QLabel(self.frame_4)
        self.label_15.setObjectName("label_15")
        self.formLayout_2.setWidget(0, QtWidgets.QFormLayout.LabelRole, self.label_15)
        self.effect = QtWidgets.QComboBox(self.frame_4)
        self.effect.setMinimumSize(self.getScaleSize(0, 25))
        self.effect.setMaximumSize(self.getScaleSize(16666, 0))
        self.effect.setStyleSheet("/*font: 12pt \"Segoe MDL2 Assets\";")
        self.effect.setObjectName("effect")
        self.formLayout_2.setWidget(0, QtWidgets.QFormLayout.FieldRole, self.effect)
        self.label_16 = QtWidgets.QLabel(self.frame_4)
        self.label_16.setMinimumSize(self.getScaleSize(75, 0))
        self.label_16.setObjectName("label_16")
        self.formLayout_2.setWidget(1, QtWidgets.QFormLayout.LabelRole, self.label_16)
        self.effect_lv = QtWidgets.QLineEdit(self.frame_4)
        self.effect_lv.setMinimumSize(self.getScaleSize(0, 25))
        self.effect_lv.setMaximumSize(self.getScaleSize(16777215, 25))
        self.effect_lv.setObjectName("effect_lv")
        self.formLayout_2.setWidget(1, QtWidgets.QFormLayout.FieldRole, self.effect_lv)
        self.label_27 = QtWidgets.QLabel(self.frame_4)
        self.label_27.setObjectName("label_27")
        self.formLayout_2.setWidget(2, QtWidgets.QFormLayout.LabelRole, self.label_27)
        self.bfres_template = QtWidgets.QLineEdit(self.frame_4)
        self.bfres_template.setMinimumSize(self.getScaleSize(0, 25))
        self.bfres_template.setMaximumSize(self.getScaleSize(16777215, 25))
        self.bfres_template.setObjectName("bfres_template")
        self.formLayout_2.setWidget(2, QtWidgets.QFormLayout.FieldRole, self.bfres_template)
        self.label = QtWidgets.QLabel(self.frame_4)
        self.label.setObjectName("label")
        self.formLayout_2.setWidget(3, QtWidgets.QFormLayout.LabelRole, self.label)
        self.bfres = QtWidgets.QLineEdit(self.frame_4)
        self.bfres.setMinimumSize(self.getScaleSize(0, 25))
        self.bfres.setMaximumSize(self.getScaleSize(16777215, 25))
        self.bfres.setObjectName("bfres")
        self.formLayout_2.setWidget(3, QtWidgets.QFormLayout.FieldRole, self.bfres)
        self.label_2 = QtWidgets.QLabel(self.frame_4)
        self.label_2.setObjectName("label_2")
        self.formLayout_2.setWidget(4, QtWidgets.QFormLayout.LabelRole, self.label_2)
        self.mainmodel = QtWidgets.QLineEdit(self.frame_4)
        self.mainmodel.setMinimumSize(self.getScaleSize(0, 25))
        self.mainmodel.setMaximumSize(self.getScaleSize(16777215, 25))
        self.mainmodel.setObjectName("mainmodel")
        self.formLayout_2.setWidget(4, QtWidgets.QFormLayout.FieldRole, self.mainmodel)
        self.label_25 = QtWidgets.QLabel(self.frame_4)
        self.label_25.setText("")
        self.label_25.setObjectName("label_25")
        self.formLayout_2.setWidget(5, QtWidgets.QFormLayout.LabelRole, self.label_25)
        self.korok_mask = QtWidgets.QCheckBox(self.frame_4)
        self.korok_mask.setMinimumSize(self.getScaleSize(0, 25))
        self.korok_mask.setObjectName("korok_mask")
        self.formLayout_2.setWidget(5, QtWidgets.QFormLayout.FieldRole, self.korok_mask)
        self.label_26 = QtWidgets.QLabel(self.frame_4)
        self.label_26.setMinimumSize(self.getScaleSize(0, 25))
        self.label_26.setMaximumSize(self.getScaleSize(16777215, 25))
        self.label_26.setObjectName("label_26")
        self.formLayout_2.setWidget(6, QtWidgets.QFormLayout.LabelRole, self.label_26)
        self.label_19 = QtWidgets.QLabel(self.frame_4)
        self.label_19.setMinimumSize(self.getScaleSize(0, 25))
        self.label_19.setText("")
        self.label_19.setObjectName("label_19")
        self.formLayout_2.setWidget(6, QtWidgets.QFormLayout.FieldRole, self.label_19)
        self.label_17 = QtWidgets.QLabel(self.frame_4)
        self.label_17.setObjectName("label_17")
        self.formLayout_2.setWidget(7, QtWidgets.QFormLayout.LabelRole, self.label_17)
        self.item1 = QtWidgets.QComboBox(self.frame_4)
        self.item1.setMinimumSize(self.getScaleSize(0, 25))
        self.item1.setObjectName("item1")
        self.formLayout_2.setWidget(7, QtWidgets.QFormLayout.FieldRole, self.item1)
        self.label_18 = QtWidgets.QLabel(self.frame_4)
        self.label_18.setObjectName("label_18")
        self.formLayout_2.setWidget(8, QtWidgets.QFormLayout.LabelRole, self.label_18)
        self.item1_n = QtWidgets.QLineEdit(self.frame_4)
        self.item1_n.setMinimumSize(self.getScaleSize(0, 25))
        self.item1_n.setMaximumSize(self.getScaleSize(16777215, 25))
        self.item1_n.setObjectName("item1_n")
        self.formLayout_2.setWidget(8, QtWidgets.QFormLayout.FieldRole, self.item1_n)
        self.label_20 = QtWidgets.QLabel(self.frame_4)
        self.label_20.setObjectName("label_20")
        self.formLayout_2.setWidget(9, QtWidgets.QFormLayout.LabelRole, self.label_20)
        self.item2 = QtWidgets.QComboBox(self.frame_4)
        self.item2.setMinimumSize(self.getScaleSize(0, 25))
        self.item2.setObjectName("item2")
        self.formLayout_2.setWidget(9, QtWidgets.QFormLayout.FieldRole, self.item2)
        self.label_21 = QtWidgets.QLabel(self.frame_4)
        self.label_21.setObjectName("label_21")
        self.formLayout_2.setWidget(10, QtWidgets.QFormLayout.LabelRole, self.label_21)
        self.item2_n = QtWidgets.QLineEdit(self.frame_4)
        self.item2_n.setMinimumSize(self.getScaleSize(0, 25))
        self.item2_n.setMaximumSize(self.getScaleSize(16777215, 25))
        self.item2_n.setObjectName("item2_n")
        self.formLayout_2.setWidget(10, QtWidgets.QFormLayout.FieldRole, self.item2_n)
        self.label_23 = QtWidgets.QLabel(self.frame_4)
        self.label_23.setObjectName("label_23")
        self.formLayout_2.setWidget(11, QtWidgets.QFormLayout.LabelRole, self.label_23)
        self.item3 = QtWidgets.QComboBox(self.frame_4)
        self.item3.setMinimumSize(self.getScaleSize(0, 25))
        self.item3.setObjectName("item3")
        self.formLayout_2.setWidget(11, QtWidgets.QFormLayout.FieldRole, self.item3)
        self.label_22 = QtWidgets.QLabel(self.frame_4)
        self.label_22.setObjectName("label_22")
        self.formLayout_2.setWidget(12, QtWidgets.QFormLayout.LabelRole, self.label_22)
        self.item3_n = QtWidgets.QLineEdit(self.frame_4)
        self.item3_n.setMinimumSize(self.getScaleSize(0, 25))
        self.item3_n.setMaximumSize(self.getScaleSize(16777215, 25))
        self.item3_n.setObjectName("item3_n")
        self.formLayout_2.setWidget(12, QtWidgets.QFormLayout.FieldRole, self.item3_n)
        self.Add_armor = QtWidgets.QPushButton(self.frame_4)
        self.Add_armor.setMinimumSize(self.getScaleSize(200, 50))
        self.Add_armor.setMaximumSize(self.getScaleSize(16777215, 16777215))
        self.Add_armor.setObjectName("Add_armor")
        self.formLayout_2.setWidget(15, QtWidgets.QFormLayout.FieldRole, self.Add_armor)
        self.Add_armors_to_items = QtWidgets.QPushButton(self.frame_4)
        self.Add_armors_to_items.setMinimumSize(self.getScaleSize(200, 50))
        self.Add_armors_to_items.setMaximumSize(self.getScaleSize(16777215, 16777215))
        self.Add_armors_to_items.setObjectName("Add_armors_to_items")
        self.formLayout_2.setWidget(14, QtWidgets.QFormLayout.FieldRole, self.Add_armors_to_items)
        self.Random_Crafting = QtWidgets.QPushButton(self.frame_4)
        self.Random_Crafting.setMinimumSize(self.getScaleSize(200, 40))
        self.Random_Crafting.setMaximumSize(self.getScaleSize(16777215, 16777215))
        self.Random_Crafting.setObjectName("Random_Crafting")
        self.formLayout_2.setWidget(13, QtWidgets.QFormLayout.FieldRole, self.Random_Crafting)
        self.horizontalLayout_5.addLayout(self.formLayout_2)
        self.horizontalLayout.addLayout(self.horizontalLayout_5)
        self.TabWidget.addTab(self.tab, "")
        self.tab_2 = QtWidgets.QWidget()
        self.tab_2.setObjectName("tab_2")
        self.frame_5 = QtWidgets.QFrame(self.tab_2)
        self.frame_5.setGeometry(self.getScaleRect(0, 0, 721, 701))
        self.frame_5.setStyleSheet("QFrame{\n"
"    background-color: rgb(81, 85, 93);\n"
"    color: white;\n"
"    \n"
"    font: 9pt \"Segoe MDL2 Assets\";\n"
"}")
        self.frame_5.setFrameShape(QtWidgets.QFrame.StyledPanel)
        self.frame_5.setFrameShadow(QtWidgets.QFrame.Raised)
        self.frame_5.setObjectName("frame_5")
        self.gridLayout_4 = QtWidgets.QGridLayout(self.frame_5)
        self.gridLayout_4.setObjectName("gridLayout_4")
        self.formLayout_11 = QtWidgets.QFormLayout()
        self.formLayout_11.setObjectName("formLayout_11")
        self.label_60 = QtWidgets.QLabel(self.frame_5)
        self.label_60.setMinimumSize(self.getScaleSize(0, 0))
        self.label_60.setMaximumSize(self.getScaleSize(16666, 16777215))
        self.label_60.setObjectName("label_60")
        self.formLayout_11.setWidget(0, QtWidgets.QFormLayout.LabelRole, self.label_60)
        self.label_63 = QtWidgets.QLabel(self.frame_5)
        self.label_63.setObjectName("label_63")
        self.formLayout_11.setWidget(1, QtWidgets.QFormLayout.LabelRole, self.label_63)
        self.magic = QtWidgets.QLineEdit(self.frame_5)
        self.magic.setMinimumSize(self.getScaleSize(0, 25))
        self.magic.setMaximumSize(self.getScaleSize(16777215, 25))
        self.magic.setObjectName("magic")
        self.formLayout_11.setWidget(1, QtWidgets.QFormLayout.FieldRole, self.magic)
        self.label_66 = QtWidgets.QLabel(self.frame_5)
        self.label_66.setObjectName("label_66")
        self.formLayout_11.setWidget(2, QtWidgets.QFormLayout.LabelRole, self.label_66)
        self.label_88 = QtWidgets.QLabel(self.frame_5)
        self.label_88.setObjectName("label_88")
        self.formLayout_11.setWidget(3, QtWidgets.QFormLayout.LabelRole, self.label_88)
        self.magicgravity = QtWidgets.QLineEdit(self.frame_5)
        self.magicgravity.setMinimumSize(self.getScaleSize(0, 25))
        self.magicgravity.setMaximumSize(self.getScaleSize(16777215, 25))
        self.magicgravity.setObjectName("magicgravity")
        self.formLayout_11.setWidget(3, QtWidgets.QFormLayout.FieldRole, self.magicgravity)
        self.label_67 = QtWidgets.QLabel(self.frame_5)
        self.label_67.setObjectName("label_67")
        self.formLayout_11.setWidget(4, QtWidgets.QFormLayout.LabelRole, self.label_67)
        self.magicspeed = QtWidgets.QLineEdit(self.frame_5)
        self.magicspeed.setMinimumSize(self.getScaleSize(0, 25))
        self.magicspeed.setMaximumSize(self.getScaleSize(16777215, 25))
        self.magicspeed.setObjectName("magicspeed")
        self.formLayout_11.setWidget(4, QtWidgets.QFormLayout.FieldRole, self.magicspeed)
        self.label_68 = QtWidgets.QLabel(self.frame_5)
        self.label_68.setObjectName("label_68")
        self.formLayout_11.setWidget(5, QtWidgets.QFormLayout.LabelRole, self.label_68)
        self.magicradius = QtWidgets.QLineEdit(self.frame_5)
        self.magicradius.setMinimumSize(self.getScaleSize(0, 25))
        self.magicradius.setMaximumSize(self.getScaleSize(16777215, 25))
        self.magicradius.setObjectName("magicradius")
        self.formLayout_11.setWidget(5, QtWidgets.QFormLayout.FieldRole, self.magicradius)
        self.label_89 = QtWidgets.QLabel(self.frame_5)
        self.label_89.setObjectName("label_89")
        self.formLayout_11.setWidget(6, QtWidgets.QFormLayout.LabelRole, self.label_89)
        self.label_90 = QtWidgets.QLabel(self.frame_5)
        self.label_90.setObjectName("label_90")
        self.formLayout_11.setWidget(7, QtWidgets.QFormLayout.LabelRole, self.label_90)
        self.label_91 = QtWidgets.QLabel(self.frame_5)
        self.label_91.setObjectName("label_91")
        self.formLayout_11.setWidget(8, QtWidgets.QFormLayout.LabelRole, self.label_91)
        self.label_92 = QtWidgets.QLabel(self.frame_5)
        self.label_92.setObjectName("label_92")
        self.formLayout_11.setWidget(9, QtWidgets.QFormLayout.LabelRole, self.label_92)
        self.label_93 = QtWidgets.QLabel(self.frame_5)
        self.label_93.setObjectName("label_93")
        self.formLayout_11.setWidget(10, QtWidgets.QFormLayout.LabelRole, self.label_93)
        self.item1_n_2 = QtWidgets.QLineEdit(self.frame_5)
        self.item1_n_2.setMinimumSize(self.getScaleSize(0, 25))
        self.item1_n_2.setMaximumSize(self.getScaleSize(16777215, 25))
        self.item1_n_2.setObjectName("item1_n_2")
        self.formLayout_11.setWidget(10, QtWidgets.QFormLayout.FieldRole, self.item1_n_2)
        self.label_94 = QtWidgets.QLabel(self.frame_5)
        self.label_94.setObjectName("label_94")
        self.formLayout_11.setWidget(11, QtWidgets.QFormLayout.LabelRole, self.label_94)
        self.label_95 = QtWidgets.QLabel(self.frame_5)
        self.label_95.setObjectName("label_95")
        self.formLayout_11.setWidget(12, QtWidgets.QFormLayout.LabelRole, self.label_95)
        self.label_96 = QtWidgets.QLabel(self.frame_5)
        self.label_96.setMinimumSize(self.getScaleSize(0, 25))
        self.label_96.setMaximumSize(self.getScaleSize(16666, 16777215))
        self.label_96.setText("")
        self.label_96.setObjectName("label_96")
        self.formLayout_11.setWidget(0, QtWidgets.QFormLayout.FieldRole, self.label_96)
        self.ismagicinf = QtWidgets.QCheckBox(self.frame_5)
        self.ismagicinf.setMinimumSize(self.getScaleSize(0, 25))
        self.ismagicinf.setText("")
        self.ismagicinf.setObjectName("ismagicinf")
        self.formLayout_11.setWidget(2, QtWidgets.QFormLayout.FieldRole, self.ismagicinf)
        self.magicrange = QtWidgets.QLineEdit(self.frame_5)
        self.magicrange.setMinimumSize(self.getScaleSize(0, 25))
        self.magicrange.setMaximumSize(self.getScaleSize(16777215, 25))
        self.magicrange.setObjectName("magicrange")
        self.formLayout_11.setWidget(6, QtWidgets.QFormLayout.FieldRole, self.magicrange)
        self.magicpower = QtWidgets.QLineEdit(self.frame_5)
        self.magicpower.setMinimumSize(self.getScaleSize(0, 25))
        self.magicpower.setMaximumSize(self.getScaleSize(16777215, 25))
        self.magicpower.setObjectName("magicpower")
        self.formLayout_11.setWidget(7, QtWidgets.QFormLayout.FieldRole, self.magicpower)
        self.label_97 = QtWidgets.QLabel(self.frame_5)
        self.label_97.setMinimumSize(self.getScaleSize(0, 25))
        self.label_97.setMaximumSize(self.getScaleSize(16666, 16777215))
        self.label_97.setText("")
        self.label_97.setObjectName("label_97")
        self.formLayout_11.setWidget(8, QtWidgets.QFormLayout.FieldRole, self.label_97)
        self.item2_n_2 = QtWidgets.QLineEdit(self.frame_5)
        self.item2_n_2.setMinimumSize(self.getScaleSize(0, 25))
        self.item2_n_2.setMaximumSize(self.getScaleSize(16777215, 25))
        self.item2_n_2.setObjectName("item2_n_2")
        self.formLayout_11.setWidget(12, QtWidgets.QFormLayout.FieldRole, self.item2_n_2)
        self.label_98 = QtWidgets.QLabel(self.frame_5)
        self.label_98.setObjectName("label_98")
        self.formLayout_11.setWidget(13, QtWidgets.QFormLayout.LabelRole, self.label_98)
        self.label_99 = QtWidgets.QLabel(self.frame_5)
        self.label_99.setObjectName("label_99")
        self.formLayout_11.setWidget(14, QtWidgets.QFormLayout.LabelRole, self.label_99)
        self.item3_n_2 = QtWidgets.QLineEdit(self.frame_5)
        self.item3_n_2.setMinimumSize(self.getScaleSize(0, 25))
        self.item3_n_2.setMaximumSize(self.getScaleSize(16777215, 25))
        self.item3_n_2.setObjectName("item3_n_2")
        self.formLayout_11.setWidget(14, QtWidgets.QFormLayout.FieldRole, self.item3_n_2)
        self.anims = QtWidgets.QCheckBox(self.frame_5)
        self.anims.setMinimumSize(self.getScaleSize(0, 25))
        self.anims.setObjectName("anims")
        self.formLayout_11.setWidget(15, QtWidgets.QFormLayout.FieldRole, self.anims)
        self.label_100 = QtWidgets.QLabel(self.frame_5)
        self.label_100.setText("")
        self.label_100.setObjectName("label_100")
        self.formLayout_11.setWidget(15, QtWidgets.QFormLayout.LabelRole, self.label_100)
        self.label_101 = QtWidgets.QLabel(self.frame_5)
        self.label_101.setText("")
        self.label_101.setObjectName("label_101")
        self.formLayout_11.setWidget(17, QtWidgets.QFormLayout.LabelRole, self.label_101)
        self.item1_2 = QtWidgets.QComboBox(self.frame_5)
        self.item1_2.setMinimumSize(self.getScaleSize(0, 25))
        self.item1_2.setObjectName("item1_2")
        self.formLayout_11.setWidget(9, QtWidgets.QFormLayout.FieldRole, self.item1_2)
        self.item2_2 = QtWidgets.QComboBox(self.frame_5)
        self.item2_2.setMinimumSize(self.getScaleSize(0, 25))
        self.item2_2.setObjectName("item2_2")
        self.formLayout_11.setWidget(11, QtWidgets.QFormLayout.FieldRole, self.item2_2)
        self.item3_2 = QtWidgets.QComboBox(self.frame_5)
        self.item3_2.setMinimumSize(self.getScaleSize(0, 25))
        self.item3_2.setObjectName("item3_2")
        self.formLayout_11.setWidget(13, QtWidgets.QFormLayout.FieldRole, self.item3_2)
        self.Add_weapon = QtWidgets.QPushButton(self.frame_5)
        self.Add_weapon.setMinimumSize(self.getScaleSize(200, 50))
        self.Add_weapon.setMaximumSize(self.getScaleSize(16777215, 16777215))
        self.Add_weapon.setObjectName("Add_weapon")
        self.formLayout_11.setWidget(17, QtWidgets.QFormLayout.FieldRole, self.Add_weapon)
        self.Random_Crafting_2 = QtWidgets.QPushButton(self.frame_5)
        self.Random_Crafting_2.setMinimumSize(self.getScaleSize(200, 40))
        self.Random_Crafting_2.setMaximumSize(self.getScaleSize(16777215, 16777215))
        self.Random_Crafting_2.setObjectName("Random_Crafting_2")
        self.formLayout_11.setWidget(16, QtWidgets.QFormLayout.FieldRole, self.Random_Crafting_2)
        self.gridLayout_4.addLayout(self.formLayout_11, 0, 1, 1, 1)
        self.verticalLayout = QtWidgets.QVBoxLayout()
        self.verticalLayout.setObjectName("verticalLayout")
        self.formLayout_10 = QtWidgets.QFormLayout()
        self.formLayout_10.setObjectName("formLayout_10")
        self.label_49 = QtWidgets.QLabel(self.frame_5)
        self.label_49.setMinimumSize(self.getScaleSize(0, 0))
        self.label_49.setMaximumSize(self.getScaleSize(16666, 16777215))
        self.label_49.setObjectName("label_49")
        self.formLayout_10.setWidget(0, QtWidgets.QFormLayout.LabelRole, self.label_49)

        self.base_2 = QtWidgets.QComboBox(self.frame_5)
        self.base_2.setMinimumSize(self.getScaleSize(0, 25))
        self.base_2.setObjectName("base_2")
        self.base_2.hide()
        # self.formLayout_10.setWidget(0, QtWidgets.QFormLayout.FieldRole, self.base_2)

        def base_2_change(i):
            self.base_2.setCurrentIndex(i)

        self.base_2_name = QtWidgets.QComboBox(self.frame_5)
        self.base_2_name.setMinimumSize(self.getScaleSize(0, 25))
        self.base_2_name.setObjectName("base_2_name")
        self.formLayout_10.setWidget(0, QtWidgets.QFormLayout.FieldRole, self.base_2_name)
        self.base_2_name.currentIndexChanged.connect(base_2_change)

        self.label_50 = QtWidgets.QLabel(self.frame_5)
        self.label_50.setObjectName("label_50")
        self.formLayout_10.setWidget(1, QtWidgets.QFormLayout.LabelRole, self.label_50)
        self.name_2 = QtWidgets.QLineEdit(self.frame_5)
        self.name_2.setMinimumSize(self.getScaleSize(0, 25))
        self.name_2.setMaximumSize(self.getScaleSize(16777215, 25))
        self.name_2.setObjectName("name_2")
        self.formLayout_10.setWidget(1, QtWidgets.QFormLayout.FieldRole, self.name_2)
        self.label_51 = QtWidgets.QLabel(self.frame_5)
        self.label_51.setObjectName("label_51")
        self.formLayout_10.setWidget(2, QtWidgets.QFormLayout.LabelRole, self.label_51)
        self.attack = QtWidgets.QLineEdit(self.frame_5)
        self.attack.setMinimumSize(self.getScaleSize(0, 25))
        self.attack.setMaximumSize(self.getScaleSize(16777215, 25))
        self.attack.setObjectName("attack")
        self.formLayout_10.setWidget(2, QtWidgets.QFormLayout.FieldRole, self.attack)
        self.label_81 = QtWidgets.QLabel(self.frame_5)
        self.label_81.setObjectName("label_81")
        self.formLayout_10.setWidget(3, QtWidgets.QFormLayout.LabelRole, self.label_81)
        self.dur = QtWidgets.QLineEdit(self.frame_5)
        self.dur.setMinimumSize(self.getScaleSize(0, 25))
        self.dur.setMaximumSize(self.getScaleSize(16777215, 25))
        self.dur.setObjectName("dur")
        self.formLayout_10.setWidget(3, QtWidgets.QFormLayout.FieldRole, self.dur)
        self.label_52 = QtWidgets.QLabel(self.frame_5)
        self.label_52.setObjectName("label_52")
        self.formLayout_10.setWidget(4, QtWidgets.QFormLayout.LabelRole, self.label_52)
        self.elink_2 = QtWidgets.QLineEdit(self.frame_5)
        self.elink_2.setMinimumSize(self.getScaleSize(0, 25))
        self.elink_2.setMaximumSize(self.getScaleSize(16777215, 25))
        self.elink_2.setObjectName("elink_2")
        self.formLayout_10.setWidget(4, QtWidgets.QFormLayout.FieldRole, self.elink_2)
        self.label_53 = QtWidgets.QLabel(self.frame_5)
        self.label_53.setObjectName("label_53")
        self.formLayout_10.setWidget(5, QtWidgets.QFormLayout.LabelRole, self.label_53)
        self.slink_2 = QtWidgets.QLineEdit(self.frame_5)
        self.slink_2.setMinimumSize(self.getScaleSize(0, 25))
        self.slink_2.setMaximumSize(self.getScaleSize(16777215, 25))
        self.slink_2.setObjectName("slink_2")
        self.formLayout_10.setWidget(5, QtWidgets.QFormLayout.FieldRole, self.slink_2)
        self.label_54 = QtWidgets.QLabel(self.frame_5)
        self.label_54.setObjectName("label_54")
        self.formLayout_10.setWidget(6, QtWidgets.QFormLayout.LabelRole, self.label_54)
        self.profile_2 = QtWidgets.QComboBox(self.frame_5)
        self.profile_2.setMinimumSize(self.getScaleSize(0, 25))
        self.profile_2.setObjectName("profile_2")
        self.formLayout_10.setWidget(6, QtWidgets.QFormLayout.FieldRole, self.profile_2)
        self.label_75 = QtWidgets.QLabel(self.frame_5)
        self.label_75.setObjectName("label_75")
        self.formLayout_10.setWidget(7, QtWidgets.QFormLayout.LabelRole, self.label_75)
        self.islifeinfinite = QtWidgets.QCheckBox(self.frame_5)
        self.islifeinfinite.setMinimumSize(self.getScaleSize(0, 25))
        self.islifeinfinite.setText("")
        self.islifeinfinite.setObjectName("islifeinfinite")
        self.formLayout_10.setWidget(7, QtWidgets.QFormLayout.FieldRole, self.islifeinfinite)
        self.label_82 = QtWidgets.QLabel(self.frame_5)
        self.label_82.setObjectName("label_82")
        self.formLayout_10.setWidget(8, QtWidgets.QFormLayout.LabelRole, self.label_82)
        self.sheath = QtWidgets.QComboBox(self.frame_5)
        self.sheath.setMinimumSize(self.getScaleSize(0, 25))
        self.sheath.setObjectName("sheath")
        self.formLayout_10.setWidget(8, QtWidgets.QFormLayout.FieldRole, self.sheath)
        self.label_56 = QtWidgets.QLabel(self.frame_5)
        self.label_56.setObjectName("label_56")
        self.formLayout_10.setWidget(9, QtWidgets.QFormLayout.LabelRole, self.label_56)
        self.physics_2 = QtWidgets.QLineEdit(self.frame_5)
        self.physics_2.setMinimumSize(self.getScaleSize(0, 25))
        self.physics_2.setMaximumSize(self.getScaleSize(16777215, 25))
        self.physics_2.setObjectName("physics_2")
        self.formLayout_10.setWidget(9, QtWidgets.QFormLayout.FieldRole, self.physics_2)
        self.label_57 = QtWidgets.QLabel(self.frame_5)
        self.label_57.setObjectName("label_57")
        self.formLayout_10.setWidget(10, QtWidgets.QFormLayout.LabelRole, self.label_57)
        self.price_2 = QtWidgets.QLineEdit(self.frame_5)
        self.price_2.setMinimumSize(self.getScaleSize(0, 25))
        self.price_2.setMaximumSize(self.getScaleSize(16777215, 25))
        self.price_2.setObjectName("price_2")
        self.formLayout_10.setWidget(10, QtWidgets.QFormLayout.FieldRole, self.price_2)
        self.label_58 = QtWidgets.QLabel(self.frame_5)
        self.label_58.setObjectName("label_58")
        self.formLayout_10.setWidget(11, QtWidgets.QFormLayout.LabelRole, self.label_58)
        self.name_desc_2 = QtWidgets.QLineEdit(self.frame_5)
        self.name_desc_2.setMinimumSize(self.getScaleSize(0, 25))
        self.name_desc_2.setMaximumSize(self.getScaleSize(16777215, 25))
        self.name_desc_2.setObjectName("name_desc_2")
        self.formLayout_10.setWidget(11, QtWidgets.QFormLayout.FieldRole, self.name_desc_2)
        self.verticalLayout.addLayout(self.formLayout_10)
        self.formLayout_5 = QtWidgets.QFormLayout()
        self.formLayout_5.setObjectName("formLayout_5")
        self.label_59 = QtWidgets.QLabel(self.frame_5)
        self.label_59.setObjectName("label_59")
        self.formLayout_5.setWidget(0, QtWidgets.QFormLayout.LabelRole, self.label_59)
        self.desc_2 = QtWidgets.QTextEdit(self.frame_5)
        self.desc_2.setMinimumSize(self.getScaleSize(0, 80))
        self.desc_2.setMaximumSize(self.getScaleSize(16777215, 100))
        self.desc_2.setStyleSheet("background-color: rgb(149, 165, 166);\n"
"font: 10pt \"Segoe MDL2 Assets\";")
        self.desc_2.setObjectName("desc_2")
        self.formLayout_5.setWidget(0, QtWidgets.QFormLayout.FieldRole, self.desc_2)
        self.verticalLayout.addLayout(self.formLayout_5)
        self.gridLayout_4.addLayout(self.verticalLayout, 0, 0, 1, 1)
        self.TabWidget.addTab(self.tab_2, "")
        self.gridLayout_2.addWidget(self.TabWidget, 1, 0, 1, 1)
        self.Upgrade_armors = QtWidgets.QPushButton(self.frame)
        self.Upgrade_armors.setEnabled(True)
        self.Upgrade_armors.setMinimumSize(self.getScaleSize(120, 50))
        self.Upgrade_armors.setMaximumSize(self.getScaleSize(16777215, 16777215))
        self.Upgrade_armors.setObjectName("Upgrade_armors")
        self.gridLayout_2.addWidget(self.Upgrade_armors, 0, 1, 1, 1)
        self.gridLayout_3.addLayout(self.gridLayout_2, 0, 0, 1, 2)
        SIC.setCentralWidget(self.centralwidget)
        self.menubar = QtWidgets.QMenuBar(SIC)
        self.menubar.setGeometry(self.getScaleRect(0, 0, 966, 25))
        self.menubar.setObjectName("menubar")
        self.menuQuit = QtWidgets.QMenu(self.menubar)
        self.menuQuit.setObjectName("menuQuit")
        self.menuMod = QtWidgets.QMenu(self.menubar)
        self.menuMod.setObjectName("menuMod")
        self.menuAbout = QtWidgets.QMenu(self.menubar)
        self.menuAbout.setObjectName("menuAbout")
        SIC.setMenuBar(self.menubar)
        self.actionOpen = QtWidgets.QAction(SIC)
        icon2 = QtGui.QIcon()
        icon2.addPixmap(QtGui.QPixmap("res/open.ico"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        self.actionOpen.setIcon(icon2)
        self.actionOpen.setObjectName("actionOpen")
        self.actionSave = QtWidgets.QAction(SIC)
        icon3 = QtGui.QIcon()
        icon3.addPixmap(QtGui.QPixmap("res/save.ico"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        self.actionSave.setIcon(icon3)
        self.actionSave.setObjectName("actionSave")
        self.actionSave_as = QtWidgets.QAction(SIC)
        self.actionSave_as.setIcon(icon3)
        self.actionSave_as.setObjectName("actionSave_as")
        self.actionQuit = QtWidgets.QAction(SIC)
        icon4 = QtGui.QIcon()
        icon4.addPixmap(QtGui.QPixmap("res/exit.ico"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        self.actionQuit.setIcon(icon4)
        self.actionQuit.setObjectName("actionQuit")
        self.actionOptions = QtWidgets.QAction(SIC)
        icon5 = QtGui.QIcon()
        icon5.addPixmap(QtGui.QPixmap("res/options.ico"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        self.actionOptions.setIcon(icon5)
        self.actionOptions.setObjectName("actionOptions")
        self.actionCreate_mod = QtWidgets.QAction(SIC)
        icon6 = QtGui.QIcon()
        icon6.addPixmap(QtGui.QPixmap("res/create.ico"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        self.actionCreate_mod.setIcon(icon6)
        self.actionCreate_mod.setObjectName("actionCreate_mod")
        self.actionClear_list = QtWidgets.QAction(SIC)
        icon7 = QtGui.QIcon()
        icon7.addPixmap(QtGui.QPixmap("res/shredder.ico"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        self.actionClear_list.setIcon(icon7)
        self.actionClear_list.setObjectName("actionClear_list")
        self.actionReadme = QtWidgets.QAction(SIC)
        self.actionReadme.setObjectName("actionReadme")
        self.menuQuit.addAction(self.actionOpen)
        self.menuQuit.addAction(self.actionSave)
        self.menuQuit.addAction(self.actionSave_as)
        self.menuQuit.addSeparator()
        self.menuQuit.addAction(self.actionOptions)
        self.menuQuit.addSeparator()
        self.menuQuit.addAction(self.actionQuit)
        self.menuMod.addAction(self.actionCreate_mod)
        self.menuMod.addAction(self.actionClear_list)
        self.menuAbout.addAction(self.actionReadme)
        self.menubar.addAction(self.menuQuit.menuAction())
        self.menubar.addAction(self.menuMod.menuAction())
        self.menubar.addAction(self.menuAbout.menuAction())

        self.retranslateUi(SIC)
        self.TabWidget.setCurrentIndex(1)
        QtCore.QMetaObject.connectSlotsByName(SIC)

    def retranslateUi(self, SIC):
        _translate = QtCore.QCoreApplication.translate
        SIC.setWindowTitle(_translate("SIC", "荒野之息独立盔甲武器制作"))
        self.edit.setText(_translate("SIC", "编辑"))
        self.label_24.setText(_translate("SIC", "模组内容"))
        self.Clear_list.setText(_translate("SIC", "清除列表"))
        self.Options.setText(_translate("SIC", "设置"))
        self.Remove_from_mod.setText(_translate("SIC", "从列表中删除"))
        self.Create_mod.setText(_translate("SIC", "创建模组 "))
        self.label_83.setText(_translate("SIC", "游戏语言"))
        self.label_84.setText(_translate("SIC", "模组名称 "))
        self.label_14.setText(_translate("SIC", "模组格式       "))
        self.wiiu_radiobutton.setText(_translate("SIC", "Wii-U"))
        self.switch_radiobutton.setText(_translate("SIC", "Switch"))
        self.label_85.setText(_translate("SIC", "商店"))
        self.label_28.setText(_translate("SIC", "模板"))
        self.label_4.setText(_translate("SIC", "装备ID"))
        self.label_5.setText(_translate("SIC", "防御力"))
        self.label_6.setText(_translate("SIC", "Elink"))
        self.label_7.setText(_translate("SIC", "Slink"))
        self.label_8.setText(_translate("SIC", "装备位置"))
        self.label_9.setText(_translate("SIC", "套装效果"))
        self.label_10.setText(_translate("SIC", "价格"))
        self.label_11.setText(_translate("SIC", "名称"))
        self.label_12.setText(_translate("SIC", "物理"))
        self.upgradeable.setText(_translate("SIC", "可升级"))
        self.label_13.setText(_translate("SIC", "描述"))
        self.label_15.setText(_translate("SIC", "效果"))
        self.label_16.setText(_translate("SIC", "效果等级"))
        self.label_27.setText(_translate("SIC", "Bfres 模板"))
        self.label.setText(_translate("SIC", "Bfres 名称"))
        self.label_2.setText(_translate("SIC", "模型名称"))
        self.korok_mask.setText(_translate("SIC", "自定义动画"))
        self.label_26.setText(_translate("SIC", "工艺"))
        self.label_17.setText(_translate("SIC", "第一材料"))
        self.label_18.setText(_translate("SIC", "数量"))
        self.label_20.setText(_translate("SIC", "第二材料"))
        self.label_21.setText(_translate("SIC", "数量"))
        self.label_23.setText(_translate("SIC", "第三材料"))
        self.label_22.setText(_translate("SIC", "数量"))
        self.Add_armor.setText(_translate("SIC", "添加盔甲"))
        self.Add_armors_to_items.setText(_translate("SIC", "将盔甲添加到列表"))
        self.Random_Crafting.setText(_translate("SIC", "随机制作要求"))
        self.TabWidget.setTabText(self.TabWidget.indexOf(self.tab), _translate("SIC", "制作盔甲"))
        self.label_60.setText(_translate("SIC", "魔法"))
        self.label_63.setText(_translate("SIC", "类型"))
        self.label_66.setText(_translate("SIC", "魔法无冷却"))
        self.label_88.setText(_translate("SIC", "重力"))
        self.label_67.setText(_translate("SIC", "速度"))
        self.label_68.setText(_translate("SIC", "半径"))
        self.label_89.setText(_translate("SIC", "范围"))
        self.label_90.setText(_translate("SIC", "伤害值"))
        self.label_91.setText(_translate("SIC", "工艺"))
        self.label_92.setText(_translate("SIC", "1. 材料"))
        self.label_93.setText(_translate("SIC", "数量"))
        self.label_94.setText(_translate("SIC", "2. 材料"))
        self.label_95.setText(_translate("SIC", "数量"))
        self.label_98.setText(_translate("SIC", "3. 材料"))
        self.label_99.setText(_translate("SIC", "数量"))
        self.anims.setText(_translate("SIC", "自定义动画"))
        self.Add_weapon.setText(_translate("SIC", "添加武器"))
        self.Random_Crafting_2.setText(_translate("SIC", "随机制作要求"))
        self.label_49.setText(_translate("SIC", "模板"))
        self.label_50.setText(_translate("SIC", "武器ID"))
        self.label_51.setText(_translate("SIC", "攻击力"))
        self.label_81.setText(_translate("SIC", "耐久度"))
        self.label_52.setText(_translate("SIC", "Elink"))
        self.label_53.setText(_translate("SIC", "Slink"))
        self.label_54.setText(_translate("SIC", "装备位置"))
        self.label_75.setText(_translate("SIC", "无限耐久"))
        self.label_82.setText(_translate("SIC", "剑鞘"))
        self.label_56.setText(_translate("SIC", "物理"))
        self.label_57.setText(_translate("SIC", "价格"))
        self.label_58.setText(_translate("SIC", "名称"))
        self.label_59.setText(_translate("SIC", "描述"))
        self.TabWidget.setTabText(self.TabWidget.indexOf(self.tab_2), _translate("SIC", "制作武器"))
        self.Upgrade_armors.setText(_translate("SIC", "升级盔甲"))
        self.menuQuit.setTitle(_translate("SIC", "文件"))
        self.menuMod.setTitle(_translate("SIC", "模组"))
        self.menuAbout.setTitle(_translate("SIC", "关于"))
        self.actionOpen.setText(_translate("SIC", "打开"))
        self.actionSave.setText(_translate("SIC", "保存"))
        self.actionSave_as.setText(_translate("SIC", "另存为"))
        self.actionQuit.setText(_translate("SIC", "退出"))
        self.actionOptions.setText(_translate("SIC", "设置"))
        self.actionCreate_mod.setText(_translate("SIC", "创建模组"))
        self.actionClear_list.setText(_translate("SIC", "清除列表"))
        self.actionReadme.setText(_translate("SIC", "关于"))
